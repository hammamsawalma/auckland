...
export function Navbar() {
    ...
    return (
        ...
        >
            <div
                className={`flex items-center justify-between lg:gap-1 xl:gap-4 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${scrolled
            ...
            >
                ...
                <nav className="hidden lg:flex items-center justify-center lg:gap-1 xl:gap-5 whitespace-nowrap shrink-0">
                    ...
                    <div className="flex items-center shrink-0">
                        {activeLocale === 'en' ? (
                            <Link href={pathname} locale="ar" aria-label="Switch to Arabic" className={`flex items-center gap-1.5 hover:text-app-acc transition-colors min-w-[44px] min-h-[44px] px-2 ${scrolled ? 'text-white/90' : 'text-white drop-shadow-md'}`}>
                            ...
                            <Link href={pathname} locale="en" aria-label="Switch to English" className={`flex items-center gap-1.5 hover:text-app-acc transition-colors min-w-[44px] min-h-[44px] px-2 ${scrolled ? 'text-white/90' : 'text-white drop-shadow-md'}`}>
                                <Globe className="w-3 h-3 xl:w-4 xl:h-4" aria-hidden="true" />
                ...
                <div className="flex items-center justify-end lg:gap-2 md:gap-4 shrink-0">
                    ...
                    <Link
                        href="/contact"
                        className={(activeLocale === 'ar' ? "font-arabic tracking-normal text-sm lg:text-base " : "lg:tracking-wider xl:tracking-[0.10em] lg:text-[10px] xl:text-[12px] ") + `hidden lg:inline-flex items-center justify-center font-bold uppercase transition-all duration-300 hover:scale-105 shrink-0 ${scrolled
                            ? "bg-app-acc text-app-dark rounded-full shadow-[0_0_15px_rgba(202,176,120,0.3)] hover:shadow-[0_0_20px_rgba(255,255,255,0.4)] hover:bg-white h-7 xl:h-9 px-3 xl:px-5 m-1"
